...
public final class OpenAICompatibleImageGenerator: VisualImageGenerating, @unchecked Sendable {
    ...
    public var isConfigured: Bool {
        guard let apiKey = keychain.loadAPIKey(for: providerId) else { return false }
        let config = configStore.load()
        return !config.openaiCompatibleBaseURL.isEmpty && !config.openaiCompatibleModel.isEmpty
    }
    ...
    public func generate(_ request: VisualGenerationRequest) async throws -> VisualGenerationResult {
        ...
        let config = configStore.load()
        guard !config.openaiCompatibleBaseURL.isEmpty else {
            throw VisualGenerationError.notConfigured(providerId: providerId)
        }

        let baseURL = config.openaiCompatibleBaseURL.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        guard let endpoint = URL(string: "\(baseURL)/v1/images/generations") else {
            throw VisualGenerationError.notConfigured(providerId: providerId)
        }
        ...
        if let dataArray = json["data"] as? [[String: Any]],
           let first = dataArray.first {
            ...
            if let b64 = first["b64_json"] as? String,
               let localPath = first["local_path"] as? String {
